import { fallbackInterpretation, getGroqClient, jsonBody, methodNotAllowed, selectedModel } from './_shared';
...
export default async function handler(req: any, res: any) {
  ...
  if (!groq) return res.status(200).json(fallbackInterpretation(body.metrics, body.algorithm, body.datasetName));
  ...
  const prompt = `Interpret the following measured ML experiment. Ground the analysis strictly in the provided metrics. Do not invent benchmarks.\n${JSON.stringify({
    experimentName: body.experimentName || 'Run #1',
    datasetName: body.datasetName || 'Dataset',
    algorithm: body.algorithm || 'ML Model',
  ...
  try {
    const completion = await groq.chat.completions.create({
      model: selectedModel(),
      messages: [
        { role: 'system', content: 'You are WiSim AI, a rigorous ML research scientist. Distinguish measured metrics from estimates.' },
